...
def make_venn_plot(data, flip=False, factor='K', traits=['Trait1', 'Trait2'], colors=[0, 1], max_size=None, formatter=None, statistic=["point_estimate"], plot_rg=True, plot_rho_beta=True, plot_cr=True):
    ...
    rho_beta = data['ci']['rho_beta'][statistic[0]]
    cr = data['ci']['fraction_concordant_within_shared'][statistic[0]]
    ...
    newline=''
    plt.title(traits[0] +' & ' + newline + traits[1], y=1.05)
    ...
    if plot_rho_beta:
        clr_rho = plt.cm.get_cmap('seismic')((rho_beta+1)/2)
        plt.gca().add_patch(patches.Rectangle(((-abs(0.7*rho_beta) if (rho_beta < 0) else 0) , -0.85), abs(0.7 * rho_beta), 0.15, fill=True, clip_on=False, color=clr_rho))
        plt.gca().add_patch(patches.Rectangle((-0.70, -0.85), 1.4, 0.15, fill=False, clip_on=False))
        plt.gca().add_patch(patches.Rectangle((0, -0.85), 0, 0.15, fill=False, clip_on=False, linewidth=3))
        plt.gca().text(-0.35 if (rho_beta>0) else 0.35, -0.85+0.13/2, '$\\rho_\\beta$={:.2f}'.format(rho_beta), fontsize=11, horizontalalignment='center', verticalalignment='center')
    if plot_cr:
        clr_cr = plt.cm.get_cmap('seismic')((cr+1)/2)
        plt.gca().add_patch(patches.Rectangle(((-abs(0.7*cr) if (cr < 0) else 0) , -1.0), abs(0.7 * cr), 0.15, fill=True, clip_on=False, color=clr_cr))
        plt.gca().add_patch(patches.Rectangle((-0.70, -1.0), 1.4, 0.15, fill=False, clip_on=False))
        plt.gca().add_patch(patches.Rectangle((0, -1.0), 0, 0.15, fill=False, clip_on=False, linewidth=3))
        plt.gca().text(-0.35 if (cr>0) else 0.35, -1.0+0.13/2, '$cr$={:.2f}'.format(cr), fontsize=11, horizontalalignment='center',         verticalalignment='center')
...
def plot_likelihood(data):
    ...
    AICmin = data['modelselection']['best_vs_min_AIC']
    AICmax = data['modelselection']['best_vs_max_AIC']
    
    plt.figtext(0.79, 0.01,
         f"AICmin={AICmin:.2f}\nAICmax={AICmax:.2f}",
...
def execute_two_parser(args):
    ...
    if match:
    	...
    	plot_likelihood(data_fit)
    	plt.title(f'Log-likelihood (Iteration {iteration_number})')
    	plt.savefig(f'log_likelihood_it{iteration_number}.png', bbox_inches='tight')
    	plt.close()
    	
    	plt.figure(figsize=(8, 6))
    	make_venn_plot(data_fit, flip=args.flip, traits=[args.trait1, args.trait2], colors=[args.trait1_color, args.trait2_color], statistic=args.statistic)
    	plt.title(f'Venn Diagram (Iteration {iteration_number})')
    	plt.savefig(f'venn_diagram_it{iteration_number}.png', bbox_inches='tight')
    	plt.close()
    else:
